...
void handleOutputRedirection(std::vector<char*>& lineTokens) {
    ...
    for (int i = 0; i < lineTokens.size() - 1; i++) {
        if (lineTokens[i] == nullptr) break;

        if (strcmp(lineTokens[i], ">") == 0) {
            if (lineTokens[i + 1] == nullptr) {
                std::cout << "No input given for redirection wont proccess" << std::endl;
                return;
            }

            outputRedirection = i;
        }
        else if (strcmp(lineTokens[i], ">>") == 0) {
    ...
    while (outputRedirection != -1) {
        ...
        if (fileStream == -1) {
            std::cout << "Can not open file: " << lineTokens[outputRedirection + 1] << ". Aborting" << std::endl;
        ...
        for (int i = 0; i < lineTokens.size() - 1; i++) {
            ...
            if (strcmp(lineTokens[i], ">") == 0) { // Classic C comparsion
                outputRedirection = i;
                break;
            }
            else if (strcmp(lineTokens[i], ">>") == 0) {
